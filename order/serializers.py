...
from account.serializers import CustomUserSerializer2
        ...
        for item_data in items_data:
            ...
            if not isinstance(product, Product):
                raise serializers.ValidationError(
                    "Invalid product provided for an order item"
                )
    ...
    def create(self, validated_data):
        ...
        if order.user:
            try:
                send_notification(
                    order.user.id,
                    f"Your order has been created with order number {order.order_number}",
                )
        ...
        print(
            f"Order finalized: Total=${total_price}, Points awarded={total_redeem_points}"
        )
        return order
